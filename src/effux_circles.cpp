...
EffuxCircles::EffuxCircles(Grapher *grapher) {
	...
	this->sdltexture = SDL_CreateTexture(grapher->sdlrenderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, width, height);
...
EffuxCircles::~EffuxCircles() {
	//if (this->pixels != nullptr) delete pixels[];	// FIXME: Segfaults
}

void EffuxCircles::execute(Uint32 time, Uint8 style) {
	Uint32 color = 0;
	...
	SDL_UpdateTexture(this->sdltexture, NULL, this->pixels, this->width * sizeof(Uint32));
}
